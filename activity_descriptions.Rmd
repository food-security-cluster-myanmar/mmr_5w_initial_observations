...
### dataset for UNDP request for Rakhine

```{r}
fsc %>% 
  filter(state == "Rakhine") %>% 
  mutate(location_type = recode(location_type, "village_ward_town" = "ward_towwn")) %>% 
...
fsc %>% filter(str_detect(hrp_indicator, "1.")) %>% 
  filter(unique_beneficiaries == "Yes") %>% 
