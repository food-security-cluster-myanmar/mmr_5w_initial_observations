...
fsc <- read_excel(
  ...
  mutate(implementing_partner_type = 
           ifelse(implementing_partners %in% c("Kaw Lah Foundation", "Hakha Baptist Association (HBA)", 
                                               "Arkan Research and Watch", "Hlaing Development Network", 
                                               "Mangrove Service Network (MSN)", "Kyal Sin May",
...
fsc %>% 
  ...
  arrange(desc(mean)) %>% 
  kable(caption = "Summary statistics of monetary values by activity;\ncontains only data from activities using cash, hybrid or voucher modalities", format.args = list(big.mark = ",")) %>% 
  kable_classic_2("striped") %>% 
...
# ggsave("partners_scatterplot.png", height = 7, width = 11, units = "in", dpi = 300)
```
...
```{r map-beneficiaries-target-township, fig.height=10}
ben %>% 
  group_by(admin3_pcode) %>% 
  summarise(beneficiaries = sum(beneficiaries)) %>% 
  right_join(pcode3_shape, by = "admin3_pcode") %>% 
  ...
  theme(legend.text = element_text(size = 10),
        legend.title = element_text(size = 10),
        legend.key.size = unit(0.7, 'cm')) +
  labs(title = "Map of number of 2021 beneficiaries by township",
       subtitle = "townships in grey have not been reached", 
       fill = "Beneficiaries") +
