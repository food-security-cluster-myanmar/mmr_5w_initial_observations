...
#          general_title = "")
 pander(caption = "Beneficiaries, townships and implementing partners by HRP, IERP and non-HRP activities")
...
#  kable_classic_2("striped") %>%  
#  footnote("`pc_of_total` only takes into account HRP and IERP beneficiaries; non-HRP figures are just for reference",
#           general_title = "")
...
#           general_title = "")
pander(caption = "COVID-19 response by HRP version")
```
...
pin %>% 
  ...
  labs(x = "",
       ...
       subtitle = "Some states/regions did not have specific HRP/IERP targets") +
...
fsc %>%  
  ...
  mutate(`%_of_total_ben` = round(total_ben / sum(total_ben) * 100, digits = 2))%>% 
  arrange(desc(total_ben)) %>%
#  kable(caption = "Top 10 townships by beneficiaries reached (desc)", format.args = list(big.mark = ",")) %>% 
...
#           general_title = "")
pander(caption = "Top 10 townships by beneficiaries reached (desc)")
...
ts_target_reached %>% 
  ...
  labs(x = "% of 2021 HRP/IERP target reached", 
       ...
       title = "Distribution of townships by percentage of 2021 HRP/IERP target reached", 
       subtitle = "Values above 200% have been lumped together at 200%;\nonly the 55 townships with specific HRP/IERP targets are reflected in this plot")
```
...
fsc %>% 
  ...
  labs(x = "Number of locations", 
       y = "Number of townships",
       title = "Distribution of townships by number of locations in each township",
       subtitle = "All townships (regardless of HRP status) have been included")
...
#  kable_classic_2() %>% 
#  footnote(general = "381,970 beneficiaries were reported in rows without specific locations",
#           general_title = "")
...
In terms of activities, `r filter(act_num, number_of_activities == 1) %>% pull(partners)` partners (`r round(filter(act_num, number_of_activities == 1) %>% pull(partners) / distinct(fsc, implementing_partners) %>% nrow() * 100, digits = 2)`% of the total)are implementing only one type of activity. Only one partner is responding across 6 activities. This indicates that it would be necessary to identify the complementary partners for beneficiaries reached by only one type of activity to achieve comprehensive food security coverage of the targetted population. 
...
fsc %>% 
  ...
  mutate(after_hrp = ifelse(date > "2021-05-01", "after_addendum", "before_addendum")) %>% 
...
has_partner <- pin %>% select(state, admin3_pcode, township, pin_new) %>% 
  left_join(ben %>%  
              group_by(admin3_pcode) %>% 
              summarise(beneficiaries = sum(beneficiaries), 
...
tsp_map_ben <- pcode3_shape %>%
  ...
  left_join(fsc %>%
              ...
              summarise(beneficiaries = sum(beneficiaries),
                        ...
                        activities = n_distinct(activity)), by = "admin3_pcode") %>% 
  left_join(pin %>% 
              select(admin3_pcode, pin_new), by = "admin3_pcode") %>% 
  ggplot() + 
  geom_sf(size = 0.1,
...
ggplotly(tsp_map_ben, tooltip = c("text")) %>%
  ...
  layout(title = list(text = paste0("Map of townships by beneficiaries reached in 2021",
                                    "<br>",
                                    "<sup>",
   donor == "WFP")                                  "mouse over for details; click and drag to select and zoom","</sup>")))
