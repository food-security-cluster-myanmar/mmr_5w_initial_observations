...
```{r}
glimpse(mmr_ts) %>% filter(sector == "Agriculture") %>% distinct(indicator_name)

mmr_ts %>% count(sector)

mmr_ts %>% filter(source_name == "General Administration Department (GAD) Township Profile") %>% distinct(indicator_name)

mmr_ts %>% filter(sector == "Environment") %>% distinct(indicator_name)
```
...
```{r}
# you can look later at interactions between activities
...
fsc %>% 
  filter(!is.na(location)) %>% 
  group_by(location, location_type, township) %>%  
  summarise(beneficiaries = sum(beneficiaries), 
            number_of_activities = n_distinct(activity), .groups = "drop") %>%  arrange(desc(number_of_activities))
```

```{r}

ts_target_reached %>% 
  mutate(gap = ifelse(beneficiaries >= target_2021, 0, target_2021 - beneficiaries)) %>% 
  summarise(gap = sum(gap),
            target_2021 = sum(target_2021)) %>% 
  mutate(gap / target_2021)
```
...
```{r}
ts_pop <- read_excel("Datasets_Vulnerability_Analysis_in_Myanmar_09Jul2018 (1).xlsx",
           skip = 1) %>% 
  slice(-c(1:3)) %>% 
  clean_names() %>% 
  remove_empty() %>% 
  select(admin1_pcode = state_region_pcode, admin3_pcode = township_pcode, pop_0_14, pop_15_64, pop_65, total_pop_both_sexes) %>%  
  pivot_longer(cols = c(pop_0_14, pop_15_64, pop_65), names_to = "disagg", values_to = "sub_pop") %>% 
  mutate_at(c("sub_pop", "total_pop_both_sexes"), as.numeric) %>% 
  mutate(pc_disagg = sub_pop / total_pop_both_sexes)

ts_pop %>% 
  group_by(admin1_pcode) %>% 
  mutate(ad = abs(pc_disagg - mean(pc_disagg))) %>% 
  summarise(aad = mean(ad, na.rm = TRUE))

# think about what you're trying to show with this graph
# i think you should be comparing admin1 against admin1
fsc_disagg %>% 
  filter(unique_beneficiaries == "Yes") %>% 
  mutate(pc_disagg = ben_sub / beneficiaries) %>% 
  group_by(admin1_pcode) %>% 
  summarise(sd = sd(pc_disagg, na.rm = TRUE)) %>% 
  inner_join(ts_pop %>% 
              group_by(admin1_pcode) %>% 
              summarise(sd2 = sd(pc_disagg, na.rm = TRUE)), by = "admin1_pcode") %>% 
  rename(fsc = sd, census = sd2) %>%
  pivot_longer(cols = c(fsc, census), names_to = "key", values_to = "sd") %>% 
  ggplot(aes(x = sd, colour = key)) +
...
Much additional work is required to improve the quality of disaggregated data collected. Disaggregated beneficiary data is one of the most common data required for internal and external reporting and it is necessary to work with partners to review their data collection and management. 
...
fsc_disagg %>% 
  ...
  group_by(implementing_partners) %>%
  mutate(ad = abs(pc_disagg - mean(pc_disagg))) %>% 
  summarise(aad = mean(ad), 
            beneficiary_frequencies = sum(ben_sub),
            locations = n_distinct(location)) %>%
  # arrange(desc(aad)) %>% {
  #   rbind(head(., 5),
  #         tail(., 5))
  # } %>%
  ggplot(aes(x = beneficiary_frequencies, y = aad)) +
  geom_point(aes(size = locations)) + 
  scale_x_continuous(trans = "log10", labels = comma) +
  geom_hline(yintercept = 0.211, colour = "red", lty = 2) +
  labs(x = "Beneficiary frequencies",
       y = "Mean absolute variance of beneficiary disaggregations", 
       title = "Mean absolute variance of beneficiary disaggregation figures by implementing partner",
       subtitle = "Each point is an implementing partner;\nThe red line is the mean level of variance based on census population figures")
