...
author: "Myanmar Food Security Cluster, 04/02/2022"
output:
  pdf_document:
    toc: yes
...
tribble(
  ...
  kbl(caption = "Acronyms", col.names = NULL, booktabs = TRUE, linesep = "") %>% 
...
hrp_version_tab %>%  
  ...
  kbl(caption = "Beneficiaries, townships and implementing partners by HRP, HRP addendum and non-HRP activities",
...
fsc %>% 
  ...
  kbl(caption = "Beneficiaries reached, by HRP indicator", format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>%
...
fsc %>%  
  ...
  kbl(caption = "Beneficiaries reached, by activity", format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position", "striped")) %>% 
  # column_spec(6, color = spec_color(hrp_act_ord$beneficiaries[1:8], end = 0.8, direction = -1, option = "G")) %>% 
  footnote("non-HRP beneficiaries have been excluded",
           general_title = "") 
...
fsc %>% filter(unique_beneficiaries == "Yes") %>% 
  ...
  kbl(caption = "COVID-19 response by HRP version", format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>% 
...
fsc %>% 
  ...
  mutate(`%_of_total_ben` = round(total_ben / sum(total_ben) * 100, digits = 2))%>% 
  arrange(desc(total_ben)) %>% 
  kbl(caption = "Beneficiaries reached (desc.) by state/region", format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
fsc %>%  
  ...
  relocate(state) %>% 
  arrange(desc(total_ben)) %>%
  kbl(caption = "Top 10 townships by beneficiaries reached (desc)", format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>% 
...
ben %>% 
  ...
  arrange(desc(beneficiaries)) %>% 
  kbl(caption = "Summary of location types", format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
fsc %>% 
  ...
  mutate(number_of_partners = recode(number_of_partners, `1` = "one",
                           ...
                           `3` = "three",
                           `4` = "four")) %>% 
  kbl(caption = "Average beneficiary frequencies in locations with one, two, three and four partners", 
        format.args = list(big.mark = ","),
        booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position"), full_width = FALSE)
...
fsc %>% 
  ...
  kbl(caption = "Average beneficiary frequencies in locations with one, two, three, four and five activities", 
        format.args = list(big.mark = ","), 
        booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"), full_width = FALSE)
...
fsc %>% 
  ...
  kbl(caption = "Beneficiary frequencies by delivery modalities and frequency of distribution", 
        format.args = list(big.mark = ","),
        booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position", "striped")) %>% 
...
fsc %>%  
  ...
  kbl(caption = "Average duration (in months) of monthly activities", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"), full_width = FALSE) %>% 
...
cash_delivery_mechanism %>% 
  kbl(caption = "Cash transfer, hybrid and voucher values per household, by cash delivery mechanism (USD)", 
        format.args = list(big.mark = ","), 
        booktabs = TRUE, linesep = "") %>% 
...
avg_transfer_value %>%  
  kbl(caption = "Average value (USD) of household package values per activity", 
        format.args = list(big.mark = ","), 
        booktabs = TRUE, linesep = "") %>% 
...
fsc %>% 
  ...
  arrange(desc(mean)) %>% 
  kbl(caption = "Summary statistics of monetary values by activity;\ncontains only data from activities using cash, hybrid or voucher modalities", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position", "striped")) %>% 
...
fsc %>% 
  ...
  mutate(pc_of_hhd = round(households / sum(households) * 100, digits = 2)) %>% 
  kbl(caption = "Monthly cash-based transfer values by percentage of MEB received", 
        format.args = list(big.mark = ","),
        booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
tribble(
  ...
  kbl(caption = "FSC partners by type", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"), full_width = FALSE)
```
...
cbind(
  ...
  fsc %>% 
    ...
    summarise(beneficiaries = sum(beneficiaries)) %>% 
    arrange(desc(beneficiaries)) %>% 
    rename(`1. # people received food and/or cash` = beneficiaries,
  ...
  fsc %>% 
    ...
    summarise(beneficiaries = sum(beneficiaries)) %>% 
    arrange(desc(beneficiaries)) %>% 
    rename(`2. # people received agri./livelihood` = beneficiaries,
           `Partners HRP indicator2` = implementing_partners) %>% 
    head(10)
  
) %>% 
  
  kbl(caption = "Top 10 implementing partners by beneficiaries reached, by HRP indicator", format.args = list(big.mark = ","),
      booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position", "striped")) %>% 
...
```{r table-act-num}
act_num %>% 
  kbl(caption = "Number of implementing partners by number of distinct activities being implemented",
        format.args = list(big.mark = ","),
        booktabs = TRUE, linesep = ""
...
fsc %>% 
  ...
  kbl(caption = "Average reach by implementing partner type", 
        format.args = list(big.mark = ","), 
        booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position")) %>% 
...
fsc %>% 
  ...
  kbl(caption = "Number of implementing partners by reporting organisation", booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position", "striped", full_width = FALSE, position = "left")) 
...
fsc %>% 
  ...
  arrange(desc(beneficiaries)) %>% 
  head(11) %>% 
  kbl(caption = "Top 10 donors by number of beneficiaries reached with their funding", 
      format.args = list(big.mark = ","), 
      booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position", "striped")) %>% 
...
fsc %>%  
  ...
  kbl(caption = "Breakdown of beneficiary types by response plan", 
      format.args = list(big.mark = ","),
      booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"))
...
fsc %>% 
  ...
  kbl(caption = "Percentage breakdown of beneficiary types by state/region", 
      format.args = list(big.mark = ","),
      booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position", "striped")) %>% 
...
pin %>%
  ...
  rename(`host_local%` = host_local_pc,
         ...
         `total%` = total_pc) %>% 
  kbl(caption = "Percentage of 2021 HRP target reached by beneficiary type", booktabs = TRUE, linesep = "") %>%
  kable_styling(latex_options = c("hold_position", "striped")) %>%
...
fsc %>% 
  ...
  rename(HRP = hrp,
         IERP = ierp,
         non_HRP = non_hrp) %>% 
  kbl(caption = "Beneficiaries reached by type, by response plan", format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("hold_position"))
...
gap_months %>%  
  ...
  summarise(partners = n_distinct(implementing_partners),
            ...
            beneficiaries = sum(ben_max)) %>% 
  arrange(desc(beneficiaries)) %>% 
  kbl(caption = "Number of beneficiaries, by activity, who have received at least 6 months of recurrent food security support",
        format.args = list(big.mark = ","), booktabs = TRUE, linesep = "") %>% 
  kable_styling(latex_options = c("HOLD_position")) %>% 
...
gap_months %>%  
  ...
  kbl(caption = "Partners who have implemented cash transfers and food baskets for at least 6 months",
      format.args = list(big.mark = ","), 
      booktabs = TRUE, linesep = "") %>% 
...
Click **[here](https://food-security-cluster-myanmar.github.io/mmr_5w_initial_observations/#7_Reference_table_%E2%80%93_townships)**. 
