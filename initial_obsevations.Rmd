...
Yangon and Rakhine form both `r round(filter(pin, state %in% c("Yangon", "Rakhine")) %>% {sum(.$target_2021)} / sum(pin$target_2021) * 100)`% of the target and `r round(filter(fsc, state %in% c("Yangon", "Rakhine") & unique_beneficiaries == "Yes") %>% {sum(.$beneficiaries)} / filter(fsc, unique_beneficiaries == "Yes") %>% {sum(.$beneficiaries)} * 100)`% of the beneficiaries reached. Mandalay has has the largest difference between targets and beneficiaries reached. There were four states (Ayeyarwady, Mon, Sagaing and Magway) where beneficiaries were reached but were not included as part of the 2021 target or PIN; however, the beneficiaries reached in these areas represent less than 5% of all beneficiaries reached. Additionally, targets have been exceeded in 7 states, with Kayin having reached 684% of its target of 6,855 persons. 
...
# you're going to need to rewrite this when you get hte Q4 data
fsc %>% 
  filter(frequency == "Monthly" & beneficiaries > 0) %>% 
